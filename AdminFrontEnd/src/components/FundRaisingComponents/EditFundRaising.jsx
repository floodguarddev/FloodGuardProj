...
import { editFundRaising } from '../../services/fund_raising.services';
...
const EditFundRaising  = ({fundRaising, setFundRaising, handleClose, open}) => {
  ...
  useEffect(()=>{
    ...
    if(fundRaising){
        ...
        setPostTitle(fundRaising.postTitle);
        setPostDescription(fundRaising.postDescription);
        setNgoId(fundRaising.ngoId);
        setNgoName(fundRaising.ngoName);
        setPostId(fundRaising._id);
        getImagesArray(fundRaising.postImagesLinks).then((prevImages)=>{
          setImages(prevImages);
        })
    }
  }, [fundRaising])

  const updateFundRaisingFunction = async (event) => {
    ...
    try{
      ...
      let response = await editFundRaising(token, ngoId, postId, formData);

      
      let editedPost = await response.data.data.news;

      setFundRaising(editedPost);
