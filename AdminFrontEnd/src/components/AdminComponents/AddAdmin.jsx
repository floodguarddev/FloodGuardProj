...
import { addAdmin } from "@/services/admins.services";
...
export function AddAdmin() {
  ...
  const [adminPhoto, setAdminPhoto] = React.useState(undefined);
  const { enqueueSnackbar } =  useSnackbar();
  const fileSelectedHandler = (event) => {
    setAdminPhoto(event.target.files[0]);
  }

  const addAdminFunc = async (event) => {
    ...
    try{
      ...
      let response = await addAdmin(token, formData);

      enqueueSnackbar("Admin Has been Created Successfully", { variant: "success" ,anchorOrigin: {
        vertical: 'bottom',
        horizontal: 'right'
      } });
          ...
          <Box sx={{mb: "10px"}}>
            <Grid container alignItems="center" justifyContent="center" spacing={3}>
              ...
              <Grid item xs={12} textAlign = "center">
                ...
                <Box mt={1}>
                    <img 
                    src={adminPhoto?URL.createObjectURL(adminPhoto):'/images/admin/defaultProfile.jpg' }
                    alt="profile" 
