...
import { getUsersList } from '../../services/users.services';
...
export const ViewUserList = ({setTotalRecords, rowsPerPage, query, handleEditOpen, usersRefresh, setUsersRefresh}) => {
  ...
  const viewUser = (userId)=>{
    setViewContext((oldValues)=>{return {...oldValues, selectedUser: userId}});
    navigate(`/users/${userId}`);
  }

  const deleteUserFunc = (userId)=>{
    deleteUser(userContext.token, userId).then(
        (response)=>{
    ...
    <>
        ...
        </TableHead>
        {
            ...
            <TableRow key={row._id} hover={true}>
                ...
                >
                    <img
                    src={row.userPhotoLink || '/images/admin/defaultProfile.jpg'}
                    alt="User"
                    width={40}
                    height={40}
